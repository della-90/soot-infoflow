 ...
 * 
 */
public class InfoflowSolver extends
		IFDSSolver<Unit, Abstraction, SootMethod, BiDiInterproceduralCFG<Unit, SootMethod>>
	...
	public InfoflowSolver(AbstractInfoflowProblem problem,
	...
	protected Set<Abstraction> computeReturnFlowFunction(
			FlowFunction<Abstraction> retFunction, Abstraction d1,
			Abstraction d2, Unit callSite,
		...
		if (retFunction instanceof SolverReturnFlowFunction) {
			...
			return ((SolverReturnFlowFunction) retFunction).computeTargets(d2,
					d1,
					callerSideDs);
		} else
	...
	protected Set<Abstraction> computeNormalFlowFunction(
			FlowFunction<Abstraction> flowFunction, Abstraction d1,
			Abstraction d2) {
		if (flowFunction instanceof SolverNormalFlowFunction)
			return ((SolverNormalFlowFunction) flowFunction).computeTargets(d1,
					d2);
		else
	...
	protected Set<Abstraction> computeCallToReturnFlowFunction(
		...
		if (flowFunction instanceof SolverCallToReturnFlowFunction)
			return ((SolverCallToReturnFlowFunction) flowFunction).computeTargets(
					d1, d2);
		else
			return flowFunction.computeTargets(d2);
	}
	...
	protected Set<Abstraction> computeCallFlowFunction(
		...
		if (flowFunction instanceof SolverCallFlowFunction)
			return ((SolverCallFlowFunction) flowFunction).computeTargets(d1,
					d2);
		else
			return flowFunction.computeTargets(d2);
	}
	...
	@Override
	public Set<Pair<Unit, Abstraction>> endSummary(SootMethod m,
			Abstraction d3) {
	...
	protected void processExit(PathEdge<Unit, Abstraction> edge) {
		...
		if (followReturnsPastSeeds && followReturnsPastSeedsHandler != null) {
			...
			final Map<Unit, Map<Abstraction, Abstraction>> inc = incoming(d1,
					methodThatNeedsSummary);

			if (inc == null || inc.isEmpty())
				followReturnsPastSeedsHandler.handleFollowReturnsPastSeeds(d1,
						u,
						d2);
		}
	}
	...
	@Override
	public void setFollowReturnsPastSeedsHandler(
			IFollowReturnsPastSeedsHandler handler) {
